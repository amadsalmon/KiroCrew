...
**Directory upload**: POST `/api/directory-upload?dir=<path>[&overwrite=1]` (multipart, one `file` part) — writes an OS file into a directory the Files panel already shows: the tree row's own drag-and-drop, the panel's full-pane drop overlay, and the row menu's "Upload files…" action all resolve their destination client-side and POST here. The handler opens with the dashboard owner gate (`require_owner_dashboard_request`, the same gate `/api/file-write` requires) before any query parsing, path probe or multipart read, so a non-owner holding a channel-created dashboard session cannot plant files into the workspace. `dir` goes through the same `_validate_dashboard_path`/`is_sensitive_path` gate `/api/file-write` trusts for writing INTO the workspace (no fixed allow-list root — any non-sensitive directory on disk), then must already exist as a directory; both that check and the destination re-check below run through `_run_path_probe` on the dedicated bounded path-probe pool, while the final write uses its path-transfer pool, never the event loop's shared `asyncio.to_thread` executor (AUTOSDE's `no-blocking-call-on-event-loop` rule is `blocking: true` — a stalled network-mounted `dir` resolving synchronously would otherwise freeze every other request and the heartbeat with it). **The destination, not just the directory, is gated**: the crew-home root is deliberately not itself sensitive (only its leaves — `security_policy.json`, `profiles/`, `admission_policy.json`, `computer_use.json` — are), so the full joined path (directory plus the uploaded filename) is re-validated through the identical gate before the extension check or the body is read, closing the case where a permitted directory plus a keystone-leaf filename would otherwise reach the keystone. Reuses `api_upload_file`'s admission gates rather than inventing new ones: the same extension allowlist (`_ALLOWED_IMAGE_EXT`/`_ALLOWED_TEXT_EXT`/`_ALLOWED_DOC_EXT`/`_ALLOWED_VIDEO_EXT`), the same `_MAX_UPLOAD_BYTES` per-file cap, and the same magic-byte content-signature check (`_content_matches_ext`, CWE-434). Deliberately does NOT run `api_upload_file`'s aggressive `[^\w.\-]` filename mangling (harmless there — every scratch upload is UUID-prefixed and never user-facing by name) or the outbound `file_send` gate's credential/secret content scan (`_gate_upload_file`'s `redact()` check exists for content LEAVING the workspace toward a third party; this endpoint writes a user's OWN file INTO their OWN workspace, the opposite trust direction, so porting that scan here would be inventing a gate rather than reusing one). One file per request by design (the frontend loops, one multipart POST per file, mirroring the Knowledge Base upload) — this is what makes per-file collision handling possible: a name collision answers 409 `code: "name_collision"` unless `overwrite=1`, so the frontend can prompt (Replace/Cancel) instead of silently overwriting.

Ancestor and publish safety, by platform. On POSIX, the write pins the destination directory's ancestor chain component-by-component (`pinned_fs.pin_parent`, walking the already-canonicalized directory rather than re-resolving it, so a directory swapped for a symlink after validation is refused). On Windows — which has no `O_NOFOLLOW` + `dir_fd` combination to pin a parent CHAIN with — the same window is closed a different way: `platform_compat.pin_directory(target_dir)` opens a handle WITHOUT `FILE_SHARE_DELETE`, and for as long as it stays open Windows refuses to rename or delete that directory OR any directory above it, so an ancestor cannot be swapped for a junction while the classify-then-publish below runs; a reparse point already sitting at `target_dir`'s own name is refused at that same open. Directory upload therefore stays AVAILABLE on Windows rather than being disabled there — the fail-closed `ancestor_pin_unsupported` (501) response only fires on a platform with neither primitive, which no host in the supported CI matrix (Linux, macOS, Windows) reaches. A fresh create publishes with `create_only=True` (`atomic_write`): `os.link`, or where hard links are unsupported (FAT/exFAT, many SMB/CIFS/FUSE mounts) `platform_compat.rename_noreplace` (`renameat2`/`renameatx_np`) — one atomic only-if-absent syscall either way, never a claim-then-replace two-step, which would only narrow a same-name race between two concurrent creates rather than remove it. Where even `rename_noreplace` is unavailable, the publish FAILS with `atomic_create_unsupported` (501) rather than complete with an unconditional replace that could silently destroy a concurrent writer's file. Overwriting an existing file instead carries its mode and access controls onto the staged replacement (mirroring `api_file_write`'s own carry): POSIX copies supported ACL xattrs from the pinned source descriptor, while Windows snapshots the source DACL, closes that source descriptor before `os.replace`, and applies the detached snapshot to the staged inode while its open descriptor pins the temp name. A Windows DACL read or apply failure refuses the overwrite with `access_control_preservation_failed` (409), leaving the original untouched. Either way the full payload is staged in a same-directory temp file first — a short write or `ENOSPC` discards the temp and leaves the original (on an overwrite) or nothing (on a fresh upload) untouched, never a truncated destination. A pre-existing symlink, junction, FIFO, socket or device at the destination name is refused rather than followed or overwritten, checked through the pinned descriptor on POSIX and by name via `platform_compat.is_link_or_junction` on Windows. Response `{"ok": true, "path", "name"}` on success.
