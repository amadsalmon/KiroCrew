...
**Directory upload**: POST `/api/directory-upload?dir=<path>[&overwrite=1]` (multipart, one `file` part) — writes an OS file into a directory the Files panel already shows: the tree row's own drag-and-drop, the panel's full-pane drop overlay, and the row menu's "Upload files…" action all resolve their destination client-side and POST here. `dir` goes through the same `_validate_dashboard_path`/`is_sensitive_path` gate `/api/file-write` trusts for writing INTO the workspace (no fixed allow-list root — any non-sensitive directory on disk), then must already exist as a directory. **The destination, not just the directory, is gated**: the crew-home root is deliberately not itself sensitive (only its leaves — `security_policy.json`, `profiles/`, `admission_policy.json`, `computer_use.json` — are), so the full joined path (directory plus the uploaded filename) is re-validated through the identical gate before the extension check or the body is read, closing the case where a permitted directory plus a keystone-leaf filename would otherwise reach the keystone. Reuses `api_upload_file`'s admission gates rather than inventing new ones: the same extension allowlist (`_ALLOWED_IMAGE_EXT`/`_ALLOWED_TEXT_EXT`/`_ALLOWED_DOC_EXT`/`_ALLOWED_VIDEO_EXT`), the same `_MAX_UPLOAD_BYTES` per-file cap, and the same magic-byte content-signature check (`_content_matches_ext`, CWE-434). Deliberately does NOT run `api_upload_file`'s aggressive `[^\w.\-]` filename mangling (harmless there — every scratch upload is UUID-prefixed and never user-facing by name) or the outbound `file_send` gate's credential/secret content scan (`_gate_upload_file`'s `redact()` check exists for content LEAVING the workspace toward a third party; this endpoint writes a user's OWN file INTO their OWN workspace, the opposite trust direction, so porting that scan here would be inventing a gate rather than reusing one). One file per request by design (the frontend loops, one multipart POST per file, mirroring the Knowledge Base upload) — this is what makes per-file collision handling possible: a name collision answers 409 `code: "name_collision"` unless `overwrite=1`, so the frontend can prompt (Replace/Cancel) instead of silently overwriting. The write itself pins the destination directory's ancestor chain component-by-component (`pinned_fs.pin_parent`, walking the already-canonicalized directory rather than re-resolving it, so a directory swapped for a symlink after validation is refused) and stages the full payload in a same-directory temp file before publishing it with one rename (`atomic_write`, through the same pinned descriptor where the platform supports descriptor-relative rename, by name otherwise) — a short write or `ENOSPC` therefore discards the temp and never leaves a truncated destination. A pre-existing symlink, junction, FIFO, socket or device at the destination name is refused rather than followed or overwritten, checked through that same pinned descriptor on POSIX and by name via `platform_compat.is_link_or_junction` on Windows, which cannot pin an ancestor chain at all. Response `{"ok": true, "path", "name"}` on success.
