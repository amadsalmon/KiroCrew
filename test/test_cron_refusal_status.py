"""A cron run whose every tool call was refused must record failure, not success.

The model still returns plausible prose when the security gate blocks every tool
it tries, so the reply text cannot carry the verdict. Recording such a run as a
success also called ``record_success()``, which resets ``consecutive_failures``
and clears ``auto_paused`` — so the auto-pause guard could never fire on a job
that was structurally incapable of succeeding, and it re-fired on its schedule
indefinitely. These tests pin the verdict AND the guard it re-enables.
"""
