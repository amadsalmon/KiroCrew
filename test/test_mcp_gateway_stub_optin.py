...
def test_a_spec_can_opt_a_server_in_directly(tmp_path: Path) -> None:
    """``poolable: true`` in the agent spec is the per-spec spelling of the stub,
    kept so a spec author can opt a server in without editing global config."""
    spec = {
        "name": "kirocrew",
        "mcpServers": {
