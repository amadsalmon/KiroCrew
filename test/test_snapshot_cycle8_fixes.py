...
class TestReplaceSavesEveryTreeBeforeReplacingAny:
    def test_a_failure_midway_has_not_replaced_anything(self, tmp_path, monkeypatch):
        ...
        block = src[src.index('if _want(components, "memory") and not _want(components, "workspace"):'):]
        block = block[: block.index("\n    try:")]
        # The save loop must appear before the replace loop.
        save_at = block.index("_copytree_safe(d, backup / tree")
        replace_at = block.index("_clear_tree_root(d)")
        assert save_at < replace_at, "replace happens before the rollback copy is complete"
        # And they must be separate loops, not one interleaved pass.
        assert block.count("for tree, d in roots:") == 2, (
            "the save and replace passes were re-interleaved"
        )
