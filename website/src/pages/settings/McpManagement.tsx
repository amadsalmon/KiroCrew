  ...
  return (
    <button
      ...
      className={[
        ...
        on ? 'bg-[var(--accent)]' : 'bg-[var(--border-strong,var(--border))]',
...
export function McpManagement() {
  ...
  const setStub = useMutation({
    ...
    onError: () => setError(i18nT('pages.mcpManagement.stub_failed')),
  })

  const setSharing = useMutation({
    mutationFn: (enabled: boolean) => api.mcpGatewayEnable(enabled),
    onSuccess: invalidate,
    onError: () => setError(i18nT('pages.mcpManagement.sharing_failed')),
  })
    ...
    <div className="space-y-4">
      ...
      <header>
        <p className="max-w-[76ch] text-[13px] leading-relaxed text-[var(--muted)]">
          {i18nT('pages.mcpManagement.lede')}{' '}
          <a
            href={DOCS_URL}
            target="_blank"
      ...
      <section className="rounded-xl border border-[var(--border)] bg-[var(--card)] px-5 py-4">
        <div className="flex items-start gap-5">
          ...
          <Switch
            on={!!status?.enabled}
            disabled={!supported || busy || statusQ.isLoading}
              ...
              return (
                <tr key={s.name} className="border-t border-[var(--border)]">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Switch
                      on={s.stub}
                      disabled={!s.can_stub || !supported || busy}
        ...
        <div className="mt-3 flex items-center justify-between gap-3 border-t border-[var(--border)] px-5 py-4">
          ...
          <div className="flex gap-2.5">
            ...
            <button
              ...
              disabled={busy}
              onClick={onConfirm}
              className="rounded-md bg-[var(--accent)] px-3.5 py-2 text-[13.5px] font-medium text-white disabled:opacity-60"
            >
