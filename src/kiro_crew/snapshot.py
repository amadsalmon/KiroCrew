...
def _restage_databases(src_dir: Path, dst_dir: Path) -> None:
    ...
    for src in sorted(src_dir.rglob("*")):
        ...
        try:
            with (
                closing(sqlite3.connect(f"file:{src}?mode=ro", uri=True)) as src_conn,
...
def _do_replace(snap: Path, mc: Path, components: list[str] | None) -> None:
    ...
    if _want(components, "memory") and not _want(components, "workspace"):
        ...
        # TWO passes, deliberately. Backing up and replacing tree-by-tree meant a
        # failure partway through (a full disk, an unreadable tree) left some trees
        # already replaced while the rollback copy of the rest was never taken — and the
        # databases were replaced before this block even started. The operator ends up
        # with memory that is half old and half new, and no complete copy of either.
        #
        # So: save everything first, and only then start replacing. A failure during the
        # save pass has changed no tree yet; a failure during the replace pass has a
        # complete rollback copy sitting in `backup`.
        roots = []
        ...
        for tree, d in roots:
            if d.is_dir():
                _copytree_safe(d, backup / tree, dirs_exist_ok=True)
