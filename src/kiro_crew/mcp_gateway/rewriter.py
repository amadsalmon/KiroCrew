    ...
    *,
    ...
    for name, entry in servers.items():
        ...
        if entry.get("disabled") is True:
            ...
            continue
        # The stub is opt-in per server. An unstubbed server passes through
        ...
        # kiro-cli's.
        if not (name in stub_servers or entry.get("poolable") is True):
            new_servers[name] = {k: v for k, v in entry.items() if k != "poolable"}
            continue
    ...
    for name, entry in inject.items():
        ...
        if not isinstance(entry, dict) or "command" not in entry:
            continue
        # The stub is opt-in here too. A settings-level server nobody stubbed is
        # left for the session to launch itself, so this path cannot reintroduce
        # the stub-per-server default through the back door.
        if not (
            name in stub_servers
            or alias in stub_servers
            or entry.get("poolable") is True
        ):
            continue
