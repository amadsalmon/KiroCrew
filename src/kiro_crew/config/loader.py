...
def _resolve_stub_servers(mcp_gateway_data: dict) -> list[str]:
    ...
    and silently falling back to a stale ``poolable_servers`` would re-route
    servers they had just cleared.

    Migrating pooled -> stubbed preserves behaviour rather than granting anything:
    a pooled server already ran behind a stub, since pooling was only reachable
    through one.
    """
